...
SCRIPT_DIR='/scratch/yp87/sl5952/defoca/jobs/T/CT/R18'
...
CEAK=$(echo "$LINE" | awk '{print $3}')
CEAGK=$(echo "$LINE" | awk '{print $4}')
if [[ -z "${ARCH:-}" ]]; then
  ARCH="resnet18"
fi
...
LOG_DIR="logs/T_CT_R18"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/${EXPID}_idx${IDX}_seed${SEED}.log"
